...
const TenantLayout: React.FC<TenantLayoutProps> = ({ children }) => {
  ...
  const [collapsed, setCollapsed] = useState<boolean>(() => localStorage.getItem('tenantSidebarCollapsed') === '1');
  const [paletteOpen, setPaletteOpen] = useState(false);
  const [notificationsOpen, setNotificationsOpen] = useState(false);
  const location = useLocation();
  const navigate = useNavigate();
  const { user, theme, toggleTheme, logout, isViewingAsTenant, viewAsSession, stopViewAs } = useAppContext();
  const { alerts } = useAlerts();
  const effectiveTenantRole = isViewingAsTenant ? viewAsSession?.tenantRole : user?.tenantRole;
  const effectiveUsername = isViewingAsTenant ? viewAsSession?.username : user?.username;
  const canSwitchOrganization = (user?.organizations?.length ?? 0) > 1;

  const topMenuItems = [
    { icon: LayoutDashboard, label: 'Dashboard', path: '/app', keywords: ['visao geral'] },
    { icon: Cpu, label: 'Dispositivos', path: '/app/devices', keywords: ['sensores', 'iot'] },
    { icon: History, label: 'Historico', path: '/app/history', keywords: ['logs', 'dados'] },
    ...(effectiveTenantRole === 'TENANT_ADMIN' || effectiveTenantRole === 'OWNER'
      ? [{ icon: Users, label: 'Usuarios', path: '/app/users', keywords: ['equipe', 'acessos'] }]
      : []),
  ];

  useEffect(() => {
    localStorage.setItem('tenantSidebarCollapsed', collapsed ? '1' : '0');
  }, [collapsed]);
    ...
    const quick: CommandItem[] = [
      {
        id: 'quick-settings',
        label: 'Configuracoes',
        hint: 'Abrir configuracoes',
        keywords: ['tema', 'ajustes'],
      ...
      ...(canSwitchOrganization
        ? [
            {
              id: 'quick-switch-org',
              label: 'Trocar organizacao',
              hint: 'Selecionar outra organizacao',
              keywords: ['cliente', 'organizacao'],
              onSelect: () => navigate('/select-organization'),
            } as CommandItem,
          ]
        : []),
    ];
    return [...quick, ...base];
  }, [canSwitchOrganization, navigate, topMenuItems]);
    ...
    <div className="min-h-screen flex text-slate-200">
        ...
        <div className={`${collapsed ? 'px-2 py-4' : 'p-6'} absolute bottom-0 left-0 right-0 z-10 border-t border-slate-800 space-y-3 bg-[#020617]`}>
              ...
              {!collapsed && (
                <div className="space-y-1">
                  ...
                  <div className="font-semibold text-white capitalize flex items-center gap-2">
                    ...
                    <span className="text-primary text-xs">{effectiveTenantRole ?? user.role}</span>
                  </div>
                </div>
              )}
              {collapsed && (
                <span className="pointer-events-none absolute left-full top-1/2 ml-3 -translate-y-1/2 whitespace-nowrap rounded-md border border-slate-700 bg-slate-900 px-2 py-1 text-xs text-slate-200 opacity-0 shadow-lg transition-opacity group-hover:opacity-100">
                  Perfil: {effectiveUsername}
                </span>
              )}
            </div>
          )}
          ...
          {canSwitchOrganization && (
            <button
              ...
              className={`group relative flex items-center ${collapsed ? 'justify-center' : 'gap-2'} text-slate-400 hover:text-secondary transition-colors text-sm font-medium w-full px-4 py-2 hover:bg-slate-800/30 rounded-lg`}
            >
              <Building2 size={16} />
              {!collapsed && <span>Trocar Organizacao</span>}
              {collapsed && (
                <span className="pointer-events-none absolute left-full top-1/2 ml-3 -translate-y-1/2 whitespace-nowrap rounded-md border border-slate-700 bg-slate-900 px-2 py-1 text-xs text-slate-200 opacity-0 shadow-lg transition-opacity group-hover:opacity-100">
                  Trocar Organizacao
                </span>
      ...
      <main className="flex-1 pt-20 lg:pt-4 px-4 lg:px-8 pb-8 overflow-x-hidden relative">
        <div className="max-w-7xl mx-auto space-y-4">
          ...
          {isViewingAsTenant && viewAsSession && (
            <div className="border border-secondary/50 bg-secondary/10 rounded-lg px-4 py-3 flex items-center justify-between gap-3">
              <div className="text-sm text-slate-200">
                Modo ver como: <span className="font-semibold">{viewAsSession.username}</span> ({viewAsSession.tenantRole})
              </div>
              <button
                onClick={() => {
                  stopViewAs();
                ...
                className="px-3 py-2 rounded-lg text-xs font-semibold bg-slate-900/60 text-secondary border border-secondary/40 hover:bg-slate-900"
              >
