...
const SettingsPage: React.FC = () => {
  ...
  useEffect(() => {
    fetchSystemStatus().then((data) => {
      setMinVal((data.minTemp ?? 2).toString());
      setMaxVal((data.maxTemp ?? 8).toString());
    });
  }, [activeTenantId]);

  const handleSave = async () => {
    const min = Number.parseFloat(minVal);
    const max = Number.parseFloat(maxVal);

    if (!Number.isFinite(min) || !Number.isFinite(max)) {
      setNotification({ msg: 'Informe valores validos para minimo e maximo.', type: 'error' });
      return;
    }

    if (min >= max) {
      setNotification({ msg: 'O valor minimo deve ser menor que o maximo.', type: 'error' });
      return;
    }

    setLoading(true);
    const ok = await updateTemperatureSettings(min, max);
    setLoading(false);

    if (ok) {
      setNotification({ msg: 'Configuracoes atualizadas com sucesso.', type: 'success' });
    } else {
      setNotification({ msg: 'Falha ao atualizar configuracoes.', type: 'error' });
    }
    ...
    <div className="max-w-5xl mx-auto space-y-8">
      <header className="pb-6 border-b border-slate-800">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-xs font-mono text-accent tracking-widest uppercase">Sistema de Configuracao</span>
        </div>
        <h2 className="text-3xl font-bold text-white mb-2">Ajustes & Configuracao</h2>
        <p className="text-slate-400">Gerencie os parametros de controle do controlador inteligente.</p>
      </header>

      <Card title="Controle de Histerese" subtitle="Defina os limites de temperatura para ativacao" className="glow-primary">
        <div className="flex flex-col lg:flex-row gap-8 mt-6">
          <div className="flex-1 space-y-6">
            <div className="space-y-2">
              <label className="text-sm font-medium text-slate-300 flex items-center gap-2 uppercase tracking-wider">
                <Thermometer size={16} className="text-primary" /> Minimo (Desliga Rele)
              </label>
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-slate-300 flex items-center gap-2 uppercase tracking-wider">
                <Thermometer size={16} className="text-danger" /> Maximo (Liga Rele)
              </label>
              <div className="relative group">
            ...
            {notification && (
              <div
                className={`p-4 rounded-lg text-sm font-medium border animate-slide-in-left ${
          ...
          <div className="lg:flex-1 glass-card rounded-2xl p-6 border border-slate-700/50 flex flex-col justify-center">
            ...
            <ul className="space-y-4 text-sm text-slate-400">
              <li className="flex gap-3">
                ...
                <span>
                  Se a temperatura <strong className="text-white">subir acima</strong> do <strong>maximo</strong>, o sistema{' '}
                  <strong className="text-primary">liga</strong> o compressor.
                </span>
              </li>
              <li className="flex gap-3">
                <div className="w-1.5 h-1.5 rounded-full bg-primary mt-1.5 flex-shrink-0" />
                <span>
                  O compressor continua ligado ate a temperatura <strong className="text-white">cair abaixo</strong> do{' '}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card
          title="Notificacoes"
          action={<div className="bg-accent/20 text-accent px-3 py-1 rounded-full text-xs font-bold border border-accent/30 tracking-wider">ATIVO</div>}
            ...
            <div className="flex items-center justify-between p-4 bg-slate-800/30 hover:bg-slate-800/50 rounded-xl border border-slate-700/30 transition-all duration-300 group cursor-pointer opacity-50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <span className="text-xs text-slate-500">Notificacoes mobile (em breve)</span>
                </div>
              </div>
        ...
        <Card title="Seguranca" className="glow-secondary">
          <div className="space-y-3 mt-4">
            <button className="w-full flex items-center justify-between p-4 bg-slate-800/30 hover:bg-slate-800/50 rounded-xl border border-slate-700/30 transition-all duration-300 group">
              ...
              <div className="text-slate-400 group-hover:text-secondary transition-colors">-&gt;</div>
            </button>
          </div>
        </Card>
      </div>
